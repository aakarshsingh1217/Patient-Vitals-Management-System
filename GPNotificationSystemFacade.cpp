...
void GPNotificationSystemFacade::updateAlertLevel(const std::string& patientID, const std::string& alertLevel) {
    if (alertLevel == "Red") {
        //std::cout << "GP Alert: Patient " << patientID << " has reached a critical alert level: " << alertLevel << std::endl;
    }
}